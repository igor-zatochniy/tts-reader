...
| `GET` | `/api/v1/voices` | Список доступних Windows SAPI голосів; потребує `X-TTS-Token`. |
...
| `-start` | empty | Фраза для старту, яка ігнорує збережену позицію після перевірки, що progress належить поточній книзі. |
